...
$categories = $funcs->GetCategories($this,0,0,false,$catscope);
...
if ($rs)
{
	...
	while ($row = $rs->FetchRow())
	{
		...
		if ($mod)
			...
			$one->item		= $neat;
		$thisid 			= $row['category_id'];
		$one->group			= $categories[$thisid]->name;
		...
		if ($mod)
		{
			...
			if ($count && ($previd == $thisid))
			{
				$one->uplink = $this->CreateLink($id,'swapitems',$returnid, $iconup,
					array('item_id'=>$one->id, 'prev_item_id'=>$prevrow->id));
				$items[$count-1]->downlink = $this->CreateLink($id,'swapitems',$returnid, $icondn,
					array('item_id'=>$prevrow->id,'next_item_id'=>$one->id));
			}
			else
			{
				$one->uplink = '';
		...
		$one->editlink = $this->CreateLink($id,'openitem',$returnid, $iconopen,
			array('item_id'=>$one->id));

		if ($pdel)
			$one->deletelink = $this->CreateLink($id,'deleteitem',$returnid, $icondel,
				array('item_id'=>$one->id),
				$this->Lang('item_confirm',$row['short_question']));
		else
			$one->deletelink = '';

		$one->selected = $this->CreateInputCheckbox($id,'selitems[]',$one->id,-1);
...
if ($rs)
{
	if ($mod && $owned)
	{
		$owners = array('&lt;'.$this->Lang('allusers').'&gt;' => '0');
		$sql = "SELECT user_id, username, first_name, last_name FROM $this->UserTable WHERE ";
		if ($allowners)
			$sql .= "admin_access=1";
		else
			$sql .= "(admin_access=1 AND user_id=$uid)"; //no injection risk
		...
		if ($rs2)
		{
			...
			$pref = cms_db_prefix();
			$sql = "SELECT DISTINCT permission_name FROM "
			.$pref."user_groups UG INNER JOIN "
			.$pref."group_perms GP ON GP.group_id = UG.group_id INNER JOIN "
			.$pref."permissions P ON P.permission_id = GP.permission_id INNER JOIN "
			.$pref."groups GR ON GR.group_id = UG.group_id WHERE UG.user_id = ? AND GR.active = 1";
			while ($row = $rs2->FetchRow())
			{
				$uid = intval($row['user_id']);
				$rs3 = $db->Execute($sql, array($uid));
				if ($rs3)
				{
					while ($row3 = $rs3->FetchRow())
					{
						$p = $row3['permission_name'];
						if (($p == $this->PermAddName) || ($p == $this->PermAdminName) || ($p == $this->PermModName))
						{
							$name = trim($row['first_name'].' '.$row['last_name']);
	...
	while ($row = $rs->FetchRow())
	{
		...
		$one->id = $row['category_id'];
		// arrange for array category_ids[] as a parameter to the callback, for matching keys
		$one->hidden = $this->CreateInputHidden($id, 'category_ids[]', $one->id);
		...
		if ($mod)
		{
			...
			if ($count)
			{
				$one->uplink = $this->CreateLink($id,'swapcats',$returnid,
					...
					array('category_id'=>$one->id, 'prev_category_id'=>$prevrow->id));
				$categories[($count-1)]->downlink = $this->CreateLink($id,'swapcats',$returnid,
					$theme->DisplayImage('icons/system/arrow-d.gif', $this->Lang('down'),'','','systemicon'),
					array('category_id'=>$prevrow->id,'next_category_id'=>$one->id));
			}
			else
				$one->uplink = '';

			if ($one->id > 0) //preserve the default category
				$one->deletelink = $this->CreateLink($id,'deletecategory',$returnid,
					...
					array('category_id'=> $one->id),
					$this->Lang('cat_confirm',$row['name']));
			else
				$one->deletelink = '';
		}
		else
		...
		$one->selected = $this->CreateInputCheckbox($id,'selgrps[]',$one->id,-1);
...
if ($mod && $extracat)
{
	...
	$one->id	= -1;
	$one->hidden		= $this->CreateInputHidden($id, 'category_ids[]', -1);
	$one->order			= count($categories)+1;
	$one->name			= '';
	$one->input_name	= $this->CreateInputText($id, 'category_names[]', '', 40);
	if ($owned)
		$one->input_owner = $this->CreateInputDropdown($id,'owner_ids[]', $owners); //, -1, $item->category_id));
	$one->selected		= $this->CreateInputCheckbox($id,'selgrps[]', -1, -2, 'class="pagecheckbox"'); //return category_id = -1 !!
	$one->downlink		= '';
	$one->uplink		= '';
...
$smarty->assign('gcount', $number);
if ($number > 0)
{
	$smarty->assign('grpitems', $categories);
	$smarty->assign('grpidtext', $this->Lang('label_id'));
	$smarty->assign('grptext', $this->Lang('category'));
	$smarty->assign('ownertext',$this->Lang('owner'));
	$smarty->assign('selectall_cats',
		$this->CreateInputCheckbox($id,'cat',true,false,'onclick="select_all(this)"'));
	//buttons
	$smarty->assign('exportbtn2', $this->CreateInputSubmit($id,'export',
		$this->Lang('export'),
		'title="'.$this->Lang('exportselcat').'" onclick="return confirm_count();"'));
	if ($pmod)
	{
		$smarty->assign('sortbtn2', $this->CreateInputSubmit($id,'sort',
			$this->Lang('sort'),
			'title="'.$this->Lang('sortselected').'" onclick="return confirm_count();"'));
		$smarty->assign('submitbtn2', $this->CreateInputSubmit($id,'update',
			$this->Lang('update'),
			'title="'.$this->Lang('updateselected').'" onclick="return confirm_count();"'));
	}
	if ($pdel)
		$smarty->assign('deletebtn2', $this->CreateInputSubmit($id,'delete',
			$this->Lang('delete'),
			'title="'.$this->Lang('deleteselcat').'" onclick="return confirm_delete();"'));
}
else
